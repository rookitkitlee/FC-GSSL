...
class GraphAutoEncoder(nn.Module):
    def __init__(self, encoder, num_atom_type=0, args=None):
        ...
        self.node_pred2 = MaskLMHead(args.embed_dim, output_dim=args.max_freqs2, activation_fn=args.task_head_activation)


    def forward(self, x, u, u2, edge_drop_pro_num, edge_drop_pro_qua, node_maks_pro_num, node_maks_pro_qua, edge_index, PE, edge_index_pe=None):


        drop_edge_union_index, drop_edge_union_pe, drop_edge_union, drop_edge_inter_index, drop_edge_inter_pe, drop_edge_inter = self.encoding_edge_drop(edge_index, edge_drop_pro_num, edge_drop_pro_qua, u, self.edge_drop)
        mask_nodes_union, x_union, mask_nodes_inter, x_inter = self.encoding_node_mask(x, node_maks_pro_num, node_maks_pro_qua, mask_ratio=self.mask_ratio, replace_ratio=self.replace_ratio)
  ...
  # kitlee
    def encoding_node_mask(self, x, node_maks_pro_num, node_maks_pro_qua, mask_ratio, replace_ratio):


        sample_num = int(mask_ratio * x.shape[0])
        mask_nodes_num = torch.multinomial(node_maks_pro_num, num_samples=sample_num, replacement=False).to(x.device)
        mask_nodes_qua = torch.multinomial(node_maks_pro_qua, num_samples=sample_num, replacement=False).to(x.device)
    ...
    # def cal_pe_loss(self, reconstruct_dis, target_dis, edge_index_pe, idx):
