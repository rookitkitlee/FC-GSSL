...
def train_mae_epoch(graph_auto_encoder, x, edge_index, edge_index_pe, u, u2, PE, edge_drop_pro_num, edge_drop_pro_qua, node_mask_pro_num, node_mask_pro_qua, optimizer):
    graph_auto_encoder.train()
    # loss = graph_auto_encoder(x, edge_index, u, u2, PE, edge_drop_pro, node_mask_pro, edge_index_pe)

    loss = graph_auto_encoder(x, u, u2, edge_drop_pro_num, edge_drop_pro_qua, node_mask_pro_num, node_mask_pro_qua, edge_index, PE, edge_index_pe)
...
for ep_num in range(args.num_exp):
    ...
    for epoch in range(1, args.epochs+1):
        ...
        train_mae_epoch(graph_auto_encoder=model, x=x, edge_index=edge, edge_index_pe=edge_index_pe, u=u, u2=u2, PE=PE, edge_drop_pro_num=edge_drop_pro_num, edge_drop_pro_qua=edge_drop_pro_qua, node_mask_pro_num=node_mask_pro_num, node_mask_pro_qua=node_mask_pro_qua,  optimizer=optimizer)
        if scheduler:
            scheduler.step()
